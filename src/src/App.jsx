...
const SAFE_OPTIONS   = ["E Wallate","Bank","à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯"];
const MEDIUM_OPTIONS = ["Gold","Land","Bond","à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯"];
const HIGH_OPTIONS   = ["Crypto","E-Commerce","Business","à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯"];

const INVEST_ICONS  = {"E Wallate":"ðŸ’³","Bank":"ðŸ¦","Gold":"ðŸ¥‡","Land":"ðŸ—ï¸","Bond":"ðŸ“œ","Crypto":"â‚¿","E-Commerce":"ðŸ›’","Business":"ðŸ¢","à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯":"ðŸ“¦"};
const INVEST_COLORS = {"E Wallate":"#4ade80","Bank":"#60a5fa","Gold":"#facc15","Land":"#fb923c","Bond":"#a78bfa","Crypto":"#f87171","E-Commerce":"#38bdf8","Business":"#e879f9","à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯":"#94a3b8"};

// â”€â”€ All world currencies â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const CURRENCIES = [
...
// â”€â”€ Default users (admin pre-creates these) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const DEFAULT_USERS = [
...
// â”€â”€ Storage helpers (in-memory per session, keyed by userId+year) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const STORE = {};
...
// â”€â”€ Expense labels â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const EXP_LABELS = {
  familySupport:"à¦«à§à¦¯à¦¾à¦®à¦¿à¦²à¦¿ à¦¸à¦¾à¦ªà§‹à¦°à§à¦Ÿ",barerKaj:"à¦¬à¦¾à¦¡à¦¼à¦¿à¦° à¦•à¦¾à¦œ",
  visaRenew:"à¦­à¦¿à¦¸à¦¾ à¦°à¦¿à¦¨à¦¿à¦‰",dharPorishod:"à¦§à¦¾à¦° à¦ªà¦°à¦¿à¦¶à§‹à¦§",
  khawarKhoroch:"à¦–à¦¾à¦“à¦¯à¦¼à¦¾à¦° à¦–à¦°à¦š",onnyo:"à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯",
};
const EXP_ICONS = {
  familySupport:"ðŸ‘¨â€ðŸ‘©â€ðŸ‘§",barerKaj:"ðŸ ",visaRenew:"ðŸ›‚",
  dharPorishod:"ðŸ’³",khawarKhoroch:"ðŸ½ï¸",onnyo:"ðŸ“¦",
};
const EXP_COLORS = {
...
function LoginScreen({users,onLogin}) {
  ...
  const handleLogin = () => {
    ...
    else setErr("âŒ à¦­à§à¦² Username à¦¬à¦¾ Password");
  };
    ...
    <div style={{...S.root,display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh"}}>
      <div style={{background:"#111827",border:"1px solid #1e293b",borderRadius:16,padding:32,width:"90%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <div style={{fontSize:"1.2rem",fontWeight:800,color:"#facc15"}}>à¦¬à¦¾à¦œà§‡à¦Ÿ à¦®à§à¦¯à¦¾à¦¨à§‡à¦œà¦¾à¦°</div>
          <div style={{fontSize:"0.75rem",color:"#94a3b8",marginTop:4}}>à¦†à¦ªà¦¨à¦¾à¦° à¦†à¦°à§à¦¥à¦¿à¦• à¦Ÿà§à¦°à§à¦¯à¦¾à¦•à¦¾à¦°à§‡ à¦¸à§à¦¬à¦¾à¦—à¦¤à¦®</div>
        </div>
        <div style={{marginBottom:14}}>
          <label style={{fontSize:"0.78rem",color:"#94a3b8",display:"block",marginBottom:5}}>ðŸ‘¤ Username</label>
          <input style={{...S.loginInput}} value={uname} onChange={e=>setUname(e.target.value)}
            placeholder="à¦†à¦ªà¦¨à¦¾à¦° username à¦¦à¦¿à¦¨" onKeyDown={e=>e.key==="Enter"&&handleLogin()} />
        </div>
        <div style={{marginBottom:20}}>
          <label style={{fontSize:"0.78rem",color:"#94a3b8",display:"block",marginBottom:5}}>ðŸ”‘ Password</label>
          <div style={{position:"relative"}}>
            <input style={{...S.loginInput,paddingRight:44}} value={pass}
              onChange={e=>setPass(e.target.value)} type={show?"text":"password"}
              placeholder="Password à¦¦à¦¿à¦¨" onKeyDown={e=>e.key==="Enter"&&handleLogin()} />
        ...
        <button onClick={handleLogin} style={S.loginBtn}>ðŸ” à¦²à¦—à¦‡à¦¨ à¦•à¦°à§à¦¨</button>
...
function BudgetManager({currentUser,users,setUsers,onLogout}) {
  ...
  const tabs = [{id:"dashboard",label:"à¦¡à§à¦¯à¦¾à¦¸à¦¬à§‹à¦°à§à¦¡"},...MONTHS_EN.map((m,i)=>({id:m,label:MONTHS[i]}))];
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        <div style={S.headerInner}>
          <div>
            <div style={S.headerTitle}>ðŸ’° à¦¬à¦¾à¦œà§‡à¦Ÿ à¦®à§à¦¯à¦¾à¦¨à§‡à¦œà¦¾à¦°</div>
            <div style={S.headerSub}>ðŸ‘¤ {currentUser.name}</div>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            {currentUser.role==="admin"&&(
              <button onClick={()=>setShowUserMgr(true)} style={S.iconBtn} title="User Manager">ðŸ‘¥</button>
            )}
            <button onClick={onLogout} style={{...S.iconBtn,background:"#7f1d1d"}} title="Logout">ðŸšª</button>
          </div>
        </div>
        {/* Live rate bar */}
        <div style={{display:"flex",alignItems:"center",gap:8,marginTop:8,fontSize:"0.7rem",color:"#94a3b8",flexWrap:"wrap"}}>
          <button onClick={fetchLiveRates} disabled={rateLoading}
            ...
            {rateLoading?"â³ à¦²à§‹à¦¡ à¦¹à¦šà§à¦›à§‡...":"ðŸ”„ à¦°à§‡à¦Ÿ à¦†à¦ªà¦¡à§‡à¦Ÿ"}
          </button>
          {rateUpdated&&<span style={{color:"#4ade80"}}>âœ“ {rateUpdated}</span>}
...
function MonthView({month,monthLabel,data,calc,updateField,toggleInvest,fmt,sym,selectedYear,currentUser}) {
  ...
  const status = calc.savingPct>=100?"ðŸŽ‰ à¦¦à¦¾à¦°à§à¦£!":calc.savingPct>=50?"ðŸ˜Š à¦­à¦¾à¦²à§‹":calc.savingPct>=20?"ðŸ˜ à¦ à¦¿à¦•à¦ à¦¾à¦•":"ðŸ˜Ÿ à¦•à¦®";

  const incRows = [
    {key:"beton",label:"à¦¬à§‡à¦¤à¦¨",icon:"ðŸ’¼"},{key:"saidIncome",label:"à¦¸à¦¾à¦‡à¦¡ à¦‡à¦¨à¦•à¦¾à¦®",icon:"ðŸ’¡"},
    {key:"onnyo",label:"à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯",icon:"âž•"},
  ];
  ...
  return (
    <div style={S.wrap}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <button onClick={handlePDF} style={S.pdfBtn}>ðŸ“„ PDF</button>
      </div>

      {/* KPIs */}
      <div style={S.kpiGrid}>
        {[{l:"à¦®à§‹à¦Ÿ à¦‡à¦¨à¦•à¦¾à¦®",v:fmt(calc.totalIncome),c:"#4ade80"},{l:"à¦®à§‹à¦Ÿ à¦–à¦°à¦š",v:fmt(calc.totalExpense),c:"#f87171"},
          {l:"à¦¸à§‡à¦­à¦¿à¦‚",v:fmt(calc.totalSaving),c:"#facc15"},{l:"à¦¸à§‡à¦­à¦¿à¦‚ %",v:`${calc.savingPct.toFixed(1)}%`,c:"#60a5fa"},
        ].map((s,i)=>(
      ...
      <div style={S.twoCol}>
        <div style={S.card}>
          <div style={{...S.cardHead,background:"linear-gradient(90deg,#065f46,#047857)"}}>ðŸ“ˆ à¦‡à¦¨à¦•à¦¾à¦®</div>
          {incRows.map(r=><InputRow key={r.key} icon={r.icon} label={r.label} value={data.income[r.key]} onChange={v=>upd("income",r.key,v)} sym={sym}/>)}
          <TotalRow label="à¦®à§‹à¦Ÿ à¦‡à¦¨à¦•à¦¾à¦®" value={fmt(calc.totalIncome)} color="#4ade80"/>
        </div>
        <div style={S.card}>
          <div style={{...S.cardHead,background:"linear-gradient(90deg,#7f1d1d,#991b1b)"}}>ðŸ“‰ à¦–à¦°à¦š</div>
          ...
          <TotalRow label="à¦®à§‹à¦Ÿ à¦–à¦°à¦š"   value={fmt(calc.totalExpense)} color="#f87171"/>
          <TotalRow label="à¦®à§‹à¦Ÿ à¦¸à§‡à¦­à¦¿à¦‚" value={fmt(calc.totalSaving)}  color="#facc15" highlight/>
        </div>
      </div>

      {/* 50-30-20 */}
      <div style={S.card}>
        <div style={{...S.cardHead,background:"linear-gradient(90deg,#1e3a5f,#1e40af)"}}>ðŸŽ¯ 50/30/20 à¦Ÿà¦¾à¦°à§à¦—à§‡à¦Ÿ</div>
        <div style={S.ruleGrid}>
          <RuleCard emoji="âœ…" title="Need"  sub="à¦¬à§‡à¦à¦šà§‡ à¦¥à¦¾à¦•à¦¾à¦° à¦œà¦¨à§à¦¯" pct="50%" val={fmt(calc.needs)}    color="#4ade80"/>
          <RuleCard emoji="ðŸŒŸ" title="Want"  sub="à¦²à¦¾à¦‡à¦« à¦à¦¨à¦œà¦¯à¦¼"        pct="30%" val={fmt(calc.wants)}    color="#facc15"/>
          <RuleCard emoji="ðŸ’°" title="Save"  sub="à¦­à¦¬à¦¿à¦·à§à¦¯à¦¤à§‡à¦° à¦œà¦¨à§à¦¯"   pct="20%" val={fmt(calc.saving20)} color="#60a5fa"/>
        </div>
      ...
      {/* Investment */}
      <div style={S.card}>
        <div style={{...S.cardHead,background:"linear-gradient(90deg,#312e81,#4c1d95)"}}>ðŸš€ à¦à¦‡ à¦®à¦¾à¦¸ à¦•à§‹à¦¥à¦¾à¦¯à¦¼ à¦°à¦¾à¦–à¦›à§‡à¦¨?</div>
        <div style={S.infoBox}>âš ï¸ à¦¬à¦¾à¦Ÿà¦¨ à¦Ÿà§à¦¯à¦¾à¦ª à¦•à¦°à§à¦¨ â€” à¦à¦•à¦¾à¦§à¦¿à¦• à¦¬à§‡à¦›à§‡ à¦¨à¦¿à¦²à§‡ à¦¸à¦®à¦¾à¦¨à¦­à¦¾à¦¬à§‡ à¦­à¦¾à¦— à¦¹à¦¬à§‡à¥¤</div>
        <InvestTierRow rank="1" color="#4ade80" tagBg="#065f46" title="Safe (à¦¨à¦¿à¦°à¦¾à¦ªà¦¦)" pct="50%"
        ...
        <TotalRow label="à¦®à§‹à¦Ÿ à¦‡à¦¨à¦­à§‡à¦¸à§à¦Ÿ" value={fmt(calc.totalInvest)} color="#a78bfa" highlight/>
      </div>
    </div>
  );
}
...
function DashboardView({dashData,grandTotal,joruriFund,investFund,platformTotals,expenseTotals,fmt,sym,selectedYear,currentUser,allData}) {
  ...
  return (
    <div style={S.wrap}>
      <div style={S.pageTitle}>ðŸ“Š à¦¡à§à¦¯à¦¾à¦¸à¦¬à§‹à¦°à§à¦¡ â€” {selectedYear}</div>

      {/* Grand KPIs */}
      <div style={{...S.kpiGrid,gridTemplateColumns:"repeat(3,1fr)"}}>
        {[
          {l:"à¦®à§‹à¦Ÿ à¦‡à¦¨à¦•à¦¾à¦®",    v:fmt(grandTotal.income),  c:"#4ade80"},
          {l:"à¦®à§‹à¦Ÿ à¦–à¦°à¦š",      v:fmt(grandTotal.expense), c:"#f87171"},
          {l:"à¦®à§‹à¦Ÿ à¦¸à§‡à¦­à¦¿à¦‚",    v:fmt(grandTotal.saving),  c:"#facc15"},
          {l:"à¦œà¦°à§à¦°à¦¿ à¦«à¦¾à¦¨à§à¦¡",  v:fmt(joruriFund),          c:"#60a5fa"},
          {l:"à¦‡à¦¨à¦­à§‡à¦¸à§à¦Ÿ à¦«à¦¾à¦¨à§à¦¡",v:fmt(investFund),          c:"#a78bfa"},
          {l:"à¦¸à§‡à¦­à¦¿à¦‚ à¦°à§‡à¦Ÿ",   v:grandTotal.income>0?`${(grandTotal.saving/grandTotal.income*100).toFixed(1)}%`:"â€”", c:"#fb923c"},
        ].map((s,i)=>(
          <div key={i} style={S.kpiCard}>
      ...
      <div style={S.card}>
        <div style={{...S.cardHead,background:"linear-gradient(90deg,#7f1d1d,#991b1b)"}}>
          ðŸ“‰ à¦–à¦°à¦š à¦•à§‹à¦¨ à¦–à¦¾à¦¤à§‡ à¦•à¦¤ à¦¯à¦¾à¦šà§à¦›à§‡ â€” {selectedYear}
        </div>
        <div style={{padding:14}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",padding:"10px 0 0",borderTop:"1px solid #334155",fontSize:"0.85rem"}}>
            <span style={{color:"#d1d5db",fontWeight:700}}>à¦®à§‹à¦Ÿ à¦–à¦°à¦š</span>
            <span style={{color:"#f87171",fontWeight:800}}>{fmt(totalExp)}</span>
          </div>
        </div>
      </div>

      {/* â”€â”€ Investment Platform Breakdown â”€â”€ */}
      <div style={S.card}>
        <div style={{...S.cardHead,background:"linear-gradient(90deg,#312e81,#4c1d95)"}}>
          ðŸ¦ à¦–à¦¾à¦¤à¦“à¦¯à¦¼à¦¾à¦°à¦¿ à¦¬à¦¿à¦¨à¦¿à¦¯à¦¼à§‹à¦— â€” à¦•à§‹à¦¥à¦¾à¦¯à¦¼ à¦•à¦¤
        </div>
        {[
          {label:"âœ… Safe",   val:safeTotal,  color:"#4ade80", platforms:SAFE_OPTIONS},
          {label:"ðŸŒŸ Medium", val:medTotal,   color:"#facc15", platforms:MEDIUM_OPTIONS},
          {label:"ðŸ”´ High",   val:highTotal,  color:"#f87171", platforms:HIGH_OPTIONS},
        ].map((tier,ti)=>(
              ...
              {tier.platforms.map(p=>{
                ...
                return (
                  <div key={p} style={{background:"#0f172a",borderRadius:10,padding:"8px 10px",
                    ...
                    <span style={{color:v>0?"#facc15":"#4b5563",fontWeight:800,fontSize:"0.82rem"}}>{v>0?fmt(v):"â€”"}</span>
        ...
        {allInvest>0&&(
          <div style={{padding:14}}>
            <div style={{color:"#94a3b8",fontSize:"0.78rem",fontWeight:700,marginBottom:10}}>ðŸ“Š à¦¤à§à¦²à¦¨à¦¾à¦®à§‚à¦²à¦• à¦šà¦¿à¦¤à§à¦°</div>
      ...
      {/* Monthly Table */}
      <div style={S.card}>
        <div style={{...S.cardHead,background:"linear-gradient(90deg,#14532d,#166534)"}}>ðŸ“… à¦®à¦¾à¦¸à¦“à¦¯à¦¼à¦¾à¦°à¦¿ à¦ªà¦¾à¦°à¦«à¦°à¦®à§à¦¯à¦¾à¦¨à§à¦¸</div>
        <div style={{overflowX:"auto"}}>
          <table style={S.table}>
            <thead><tr>{["à¦®à¦¾à¦¸","à¦‡à¦¨à¦•à¦¾à¦®","à¦–à¦°à¦š","à¦¸à§‡à¦­à¦¿à¦‚","Safe","Medium","High","à¦¸à§‡à¦­à¦¿à¦‚%"].map(h=>
            ...
            <tbody>
              {dashData.map((d,i)=>(
                <tr key={i} style={{background:i%2===0?"#1a2332":"#111827"}}>
                  ...
                  <td style={{...S.td,color:"#4ade80"}}>{d.totalIncome>0?fmt(d.totalIncome):"â€”"}</td>
                  <td style={{...S.td,color:"#f87171"}}>{d.totalExpense>0?fmt(d.totalExpense):"â€”"}</td>
                  <td style={{...S.td,color:"#60a5fa",fontWeight:600}}>{d.totalSaving>0?fmt(d.totalSaving):"â€”"}</td>
                  <td style={{...S.td,color:"#4ade80"}}>{d.investSafe>0?fmt(d.investSafe):"â€”"}</td>
                  <td style={{...S.td,color:"#facc15"}}>{d.investMedium>0?fmt(d.investMedium):"â€”"}</td>
                  <td style={{...S.td,color:"#f87171"}}>{d.investHigh>0?fmt(d.investHigh):"â€”"}</td>
                  <td style={{...S.td,color:d.savingPct>=20?"#4ade80":"#f87171"}}>
                    {d.totalIncome>0?`${d.savingPct.toFixed(1)}%`:"â€”"}</td>
                </tr>
              ))}
              <tr style={{background:"#0f172a",borderTop:"2px solid #facc15"}}>
                {[{v:"à¦®à§‹à¦Ÿ",c:"#facc15"},{v:fmt(grandTotal.income),c:"#4ade80"},
                  ...
                  {v:grandTotal.income>0?`${(grandTotal.saving/grandTotal.income*100).toFixed(1)}%`:"â€”",c:"#a78bfa"},
...
function UserManager({users,setUsers,onClose}) {
  ...
  const addUser=()=>{
    if(!newU.username||!newU.password||!newU.name){setErr("à¦¸à¦¬ à¦«à¦¿à¦²à§à¦¡ à¦ªà§‚à¦°à¦£ à¦•à¦°à§à¦¨");return;}
    if(users.find(u=>u.username===newU.username)){setErr("Username à¦‡à¦¤à¦¿à¦®à¦§à§à¦¯à§‡ à¦†à¦›à§‡");return;}
    const id="u"+Date.now();
    ...
    <div style={{position:"fixed",inset:0,background:"#000000bb",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#111827",border:"1px solid #334155",borderRadius:16,padding:24,width:"90%",maxWidth:480,maxHeight:"85vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
          <div style={{color:"#facc15",fontWeight:800,fontSize:"1rem"}}>ðŸ‘¥ User Manager</div>
        ...
        <div style={{background:"#0f172a",borderRadius:10,padding:14,marginBottom:14}}>
          <div style={{color:"#60a5fa",fontWeight:700,fontSize:"0.82rem",marginBottom:10}}>âž• à¦¨à¦¤à§à¦¨ à¦‡à¦‰à¦œà¦¾à¦° à¦¯à§‹à¦— à¦•à¦°à§à¦¨</div>
          {[{p:"à¦¨à¦¾à¦®",k:"name"},{p:"Username",k:"username"},{p:"Password",k:"password"}].map(f=>(
          ...
          <button onClick={addUser} style={S.loginBtn}>à¦‡à¦‰à¦œà¦¾à¦° à¦¯à§‹à¦— à¦•à¦°à§à¦¨</button>
        </div>
        {/* User list */}
        <div style={{color:"#94a3b8",fontSize:"0.78rem",marginBottom:8}}>à¦¬à¦°à§à¦¤à¦®à¦¾à¦¨ à¦‡à¦‰à¦œà¦¾à¦° ({users.length})</div>
        {users.map(u=>(
...
function generatePDF({month,monthLabel,data,calc,fmt,sym,selectedYear,currentUser}) {
  ...
  <h1>ðŸ’° à¦¬à¦¾à¦œà§‡à¦Ÿ à¦°à¦¿à¦ªà§‹à¦°à§à¦Ÿ â€” ${monthLabel} ${selectedYear}</h1>
  <div class="sub">ðŸ‘¤ ${currentUser.name} &nbsp;|&nbsp; à¦®à§à¦¦à§à¦°à¦¾: ${sym} &nbsp;|&nbsp; à¦¤à§ˆà¦°à¦¿: ${new Date().toLocaleString()}</div>

  <div class="grid">
    <div class="card"><div class="val">${fmt(calc.totalIncome)}</div><div class="lbl">à¦®à§‹à¦Ÿ à¦‡à¦¨à¦•à¦¾à¦®</div></div>
    <div class="card"><div class="val">${fmt(calc.totalExpense)}</div><div class="lbl">à¦®à§‹à¦Ÿ à¦–à¦°à¦š</div></div>
    <div class="card"><div class="val">${fmt(calc.totalSaving)}</div><div class="lbl">à¦¸à§‡à¦­à¦¿à¦‚</div></div>
    <div class="card"><div class="val">${calc.savingPct.toFixed(1)}%</div><div class="lbl">à¦¸à§‡à¦­à¦¿à¦‚ à¦°à§‡à¦Ÿ</div></div>
  </div>

  <div class="section">ðŸ“ˆ à¦‡à¦¨à¦•à¦¾à¦®</div>
  <table><tr><th>à¦–à¦¾à¦¤</th><th>à¦ªà¦°à¦¿à¦®à¦¾à¦£</th></tr>
    <tr><td>à¦¬à§‡à¦¤à¦¨</td><td>${fmt(num(data.income.beton))}</td></tr>
    <tr><td>à¦¸à¦¾à¦‡à¦¡ à¦‡à¦¨à¦•à¦¾à¦®</td><td>${fmt(num(data.income.saidIncome))}</td></tr>
    <tr><td>à¦…à¦¨à§à¦¯à¦¾à¦¨à§à¦¯</td><td>${fmt(num(data.income.onnyo))}</td></tr>
    <tr><td><b>à¦®à§‹à¦Ÿ</b></td><td><b>${fmt(calc.totalIncome)}</b></td></tr>
  </table>

  <div class="section">ðŸ“‰ à¦–à¦°à¦š</div>
  <table><tr><th>à¦–à¦¾à¦¤</th><th>à¦ªà¦°à¦¿à¦®à¦¾à¦£</th><th>%</th></tr>
    ...
    <tr><td><b>à¦®à§‹à¦Ÿ</b></td><td><b>${fmt(calc.totalExpense)}</b></td><td>100%</td></tr>
  </table>

  <div class="section">ðŸŽ¯ 50/30/20 à¦Ÿà¦¾à¦°à§à¦—à§‡à¦Ÿ</div>
  <table><tr><th>à¦•à§à¦¯à¦¾à¦Ÿà¦¾à¦—à¦°à¦¿</th><th>à¦Ÿà¦¾à¦°à§à¦—à§‡à¦Ÿ</th><th>à¦ªà¦°à¦¿à¦®à¦¾à¦£</th></tr>
    <tr><td>âœ… Need (50%)</td><td>50%</td><td>${fmt(calc.needs)}</td></tr>
    <tr><td>ðŸŒŸ Want (30%)</td><td>30%</td><td>${fmt(calc.wants)}</td></tr>
    <tr><td>ðŸ’° Save (20%)</td><td>20%</td><td>${fmt(calc.saving20)}</td></tr>
  </table>

  <div class="section">ðŸš€ à¦‡à¦¨à¦­à§‡à¦¸à§à¦Ÿà¦®à§‡à¦¨à§à¦Ÿ</div>
  <table><tr><th>à¦Ÿà¦¾à¦¯à¦¼à¦¾à¦°</th><th>%</th><th>à¦ªà¦°à¦¿à¦®à¦¾à¦£</th><th>à¦ªà§à¦²à§à¦¯à¦¾à¦Ÿà¦«à¦°à§à¦®</th></tr>
    ...
    <tr><td><b>à¦®à§‹à¦Ÿ</b></td><td></td><td><b>${fmt(calc.totalInvest)}</b></td><td></td></tr>
  </table>

  <div class="footer">à¦¬à¦¾à¦œà§‡à¦Ÿ à¦®à§à¦¯à¦¾à¦¨à§‡à¦œà¦¾à¦° â€” ${currentUser.name} â€” ${monthLabel} ${selectedYear}</div>
  </body></html>`;
        ...
        {options.map(opt=>{
          ...
          return (
            <button key={opt} onClick={()=>onToggle(opt)} style={{
              ...
              {INVEST_ICONS[opt]} {opt} {active?"âœ“":""}
            </button>
          );
        })}
      </div>
